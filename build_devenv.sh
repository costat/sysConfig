...
zsh_version=5.8
gcc_version=10.2.0
llvm_release_version=10.0.1
llvm_head_version=${date_version}
openblas_version=${date_version}
openmpi_3_version=3.1.6
openmpi_4_version=4.0.4
...
if [[ $BUILD_MODULES = 'true' ]] ; then
  ...
  wget http://prdownloads.sourceforge.net/modules/modules-${modules_version}.tar.gz
...
if [[ $BUILD_OPENMPI_4 = 'true' ]] ; then
  ...
  wget https://www.open-mpi.org/software/ompi/v4.0/downloads/openmpi-${openmpi_4_version}.tar.bz2
  tar xvjf openmpi-${openmpi_4_version}.tar.bz2
  cd openmpi-${openmpi_4_version}

  ./configure --prefix=${local_path_apps}/openmpi-${openmpi_4_version}
  make -j${par_build} all
  make install
